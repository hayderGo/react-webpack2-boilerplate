...
export default class PieReact extends Component {
    ...
    return {
      ...
      xAxis: {
        ...
        axisLabel: {
          ...
          //   if (index === 0 ||
          //     (index + 1) % (Math.ceil(dates.length / 2)) === 0 ||
  ...
  initPie() {
    if (!echarts.getInstanceByDom(this.div)) {
      this.myChart = echarts.init(this.div); // 初始化echarts
    } else {
      this.myChart = echarts.getInstanceByDom(this.div);
    }
...
let data = rawData.map((item) => [
  parseFloat(item[1]), parseFloat(item[2]), parseFloat(item[5]), parseFloat(item[6])
]);
